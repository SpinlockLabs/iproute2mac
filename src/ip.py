...
def do_route_get(argv, af):
    ...
    res = dict(re.findall(
        '^\W*((?:route to|destination|gateway|interface)): (.+)$', res, re.MULTILINE))
    ...
    try:
        ...
        for arg in args:
            if arg == 'up':
                ...
                if not execute_cmd(SUDO + " " + IFCONFIG + " " + dev + " down"):
                    return False
                ...
                if addr in ['random', 'rand']:
                    ...
                    if status != 0:
                        return False
                    details = re.findall(
                        '^(?:Device|Ethernet Address): (.+)$', res, re.MULTILINE)
                    addr = details[details.index(dev)+1]
                if not execute_cmd(SUDO + " " + IFCONFIG + " " + dev + " lladdr " + addr):
                    return False
            elif arg == 'mtu':
                mtu = int(args.next())
                if not execute_cmd(SUDO + " " + IFCONFIG + " " + dev + " mtu " + str(mtu)):
                    return False
    except:
        return False
    return True
